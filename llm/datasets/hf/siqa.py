...
def format_sample_prompt(prompt_dataset, format, kshot=1, seed=None):
    ...
    if format == PromptFormat.CHOICE:
        prompt = [
            "The following are social interaction questions with multiple-choice answers.\n",
            *fewshot_samples_prompt,
            "Now, answer the next question.\n\n",
        ]
    elif format == PromptFormat.OE:
        prompt = [
            "The following are social interaction questions with answers.\n",
            *fewshot_samples_prompt,
            "Now, answer the next question.\n\n",
        ]
    else:
...
@register_dataset
def siqa(*args, **kwargs):
    return get_siqa(*args, **kwargs)
