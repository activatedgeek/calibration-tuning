...
def format_sample(sample, format, with_query_label=False, seed=None):
    ...
    if format == PromptFormat.CHOICE:
        ...
        context = "\n".join(
            [
                'Read the following paragraph along with the question and answer. Then, respond with whether the answer is correct. Respond with only "Yes" or "No" and no additional text.',
                f"Passage: {paragraph}",
                f"Question: {question}",
                f"Answer: {answer}. Is the answer correct?",
            ]
        )
...
def format_sample_prompt(prompt_dataset, format, kshot=1, seed=None):
    ...
    if format == PromptFormat.CHOICE:
        prompt = [
            "The following are reading comprehensions (with multiple-choice answers).\n",
            *fewshot_samples_prompt,
...
def multirc(*args, **kwargs):
    return get_multirc(*args, **kwargs)
