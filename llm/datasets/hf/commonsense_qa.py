...
def format_sample(sample, format, with_query_label=False, seed=None):
    ...
    target_idx = string.ascii_lowercase.index(sample["answerKey"].lower())
...
def get_commonsense_qa(
    ...
    **_,
    ...
    dataset = dataset.filter(
        lambda x: len(x["choices"]["text"]) == len(np.unique(x["choices"]["text"])),
...
def commonsense_qa(*args, **kwargs):
    return get_commonsense_qa(*args, **kwargs)
