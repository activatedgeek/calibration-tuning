...
def format_sample(sample, format, with_query_label=False, seed=None):
    ...
    if format == PromptFormat.CHOICE:
        ...
        context = "\n".join(
            [
                'Read the following premise and answer if the hypothesis is true. Respond only with "Yes", "No", or "It\'s impossible to say" and no additional text.',
                premise,
...
def get_cb(
    ...
    **_,
    ...
    dataset.pop("test", None)  ## NOTE: Test does not have labels.
