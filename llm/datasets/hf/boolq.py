...
def format_sample(sample, format, with_query_label=False, seed=None):
    ...
    target_idx = int(bool(sample["answer"]))
    ...
    if format == PromptFormat.CHOICE:
        ...
        context = "\n".join(
            [
                'Read the following passage and answer the question. Respond with only "True" or "False" and no additional text.',
                f"Passage: {passage}",
                f"Question: {question}?",
            ]
        )
