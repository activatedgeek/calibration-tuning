...
def format_sample(sample, format, with_query_label=False, seed=None):
    ...
    sample["answerKey"] = (
        string.ascii_lowercase.index(sample["answerKey"])
        if sample["answerKey"] in string.ascii_lowercase
        else int(sample["answerKey"]) - 1
    )
...
@register_dataset
def arc(*args, **kwargs):
    return get_arc(*args, **kwargs, subset="ARC-Easy")


@register_dataset
def arc_challenge(*args, **kwargs):
    return get_arc(*args, **kwargs, subset="ARC-Challenge")
